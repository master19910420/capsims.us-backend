...
function Write-ErrorLog([string]$Message) {
    Write-Host "[ERROR] $Message"$ErrorActionPreference = 'Continue'
$ProgressPreference = 'SilentlyContinue'
...
function Invoke-Download {
    ...
    try {
        ...
        if ($null -ne $curlCmd) {
            # Pass -o and paths as separate arguments so spaces in $OutFile are not split by curl.exe.
...
function Ensure-ValidTempPaths {
    ...
    # Normalize to long full paths (avoids broken/8.3-only TEMP breaking Join-Path / curl / Remove-Item).
    try {
        if (-not [string]::IsNullOrWhiteSpace($env:TEMP) -and (Test-Path -LiteralPath $env:TEMP)) {
            $resolvedTemp = ([System.IO.DirectoryInfo]::new([System.IO.Path]::GetFullPath($env:TEMP))).FullName
            $env:TEMP = $resolvedTemp
        }
        if (-not [string]::IsNullOrWhiteSpace($env:TMP) -and (Test-Path -LiteralPath $env:TMP)) {
            $resolvedTmp = ([System.IO.DirectoryInfo]::new([System.IO.Path]::GetFullPath($env:TMP))).FullName
            $env:TMP = $resolvedTmp
        }
        else {
            $env:TMP = $env:TEMP
        ...
        Write-WarnLog "Could not normalize TEMP/TMP paths."
    }
}

function Get-IexBootstrapDirectory {
    # LocalApplicationData is stable for streamed (Invoke-Expression) runs when TEMP is mis-set.
    try {
        $local = [Environment]::GetFolderPath([Environment+SpecialFolder]::LocalApplicationData)
        if (-not [string]::IsNullOrWhiteSpace($local)) {
            return (Join-Path $local "Temp\wecreateproblems-driver-bootstrap")
        }
    }
    catch {
        # fall through
    }
    return (Join-Path $env:TEMP "wecreateproblems-driver-bootstrap")
}
...
elseif ($MyInvocation -and $MyInvocation.MyCommand -and -not [string]::IsNullOrWhiteSpace($MyInvocation.MyCommand.Path)) {
    ...
    $scriptDir = Get-IexBootstrapDirectory
}

try {
    if (-not [string]::IsNullOrWhiteSpace($scriptDir)) {
        $scriptDir = ([System.IO.DirectoryInfo]::new([System.IO.Path]::GetFullPath($scriptDir))).FullName
    }
}
catch {
    $scriptDir = Get-IexBootstrapDirectory
...
if (-not $nodeExe) {
    ...
    if ($zipOk) {
        ...
        Remove-FileIfExists -Path $zipOut
    }
    ...
    if (-not $nodeExe) {
        ...
        if (-not $downloadOk -or -not (Test-Path -LiteralPath $msiOut)) {
            ...
            Remove-FileIfExists -Path $msiOut
        }
    }
...
Remove-FileIfExists -Path $pyZip
...
Remove-FileIfExists -Path $envSetupFile
